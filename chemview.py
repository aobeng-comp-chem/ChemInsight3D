...
from PyQt5.QtCore import Qt, QTimer, QSize, QPropertyAnimation, QEasingCurve, QRect, QThread, pyqtSignal, QEvent, QObject
...
class _DialogCenterFilter(QObject):
    """
    App-wide event filter that centers every QDialog (including QMessageBox,
    QFileDialog, QColorDialog, QProgressDialog, etc. -- all QDialog subclasses)
    on its screen instead of leaving placement to the window manager.

    Qt sends a hidden Move event after a dialog has its final layout size but
    before its first Show event. Centering there avoids both random placement
    and a visible post-show jump.
    """
    ...
    def eventFilter(self, obj, event):
        ...
        centering = bool(obj.property("_centering_in_progress"))
        if (event.type() in (QEvent.Move, QEvent.Resize) and
                not obj.isVisible() and
                not centering):
            self.center(obj)
        elif event.type() == QEvent.Show and not centering:
            # Fallback for a platform/dialog implementation that does not
            # deliver the hidden Move event.
            self.center(obj)
        return False

    @staticmethod
    def center(dialog):
        """Size and position a dialog before its first visible frame."""
        try:
            # Guard only the recursive Move/Resize events generated by this
            # method. A later layout resize must be allowed to recenter again.
            dialog.setProperty("_centering_in_progress", True)
            dialog.ensurePolished()
            dialog.adjustSize()
            parent = dialog.parentWidget()
            screen = (
                dialog.screen()
                or (parent.screen() if parent is not None else None)
            ...
            # The dialog may have been deleted while an event is in flight.
            pass
        finally:
            try:
                dialog.setProperty("_centering_in_progress", False)
            except RuntimeError:
                pass
...
# Molecular geometry is deliberately denser than the contour grid so atom
...
class _SpinDensityComputeThread(QThread):
    ...
    def __init__(self, path, grid_quality, ext_dist, density_types, parent=None):
        super().__init__(parent)
        self.path          = path
        self.grid_quality  = grid_quality
        self.ext_dist      = ext_dist
        self.density_types = list(density_types)
    ...
    def run(self):
        try:
            ...
            self.progress.emit(f"Computing {', '.join(self.density_types)} density…")
            result = _da.compute_spin_density_cube_data(
                self.path, grid_quality=self.grid_quality,
                ext_dist=self.ext_dist, bohr_const=bohr_const,
                density_type=self.density_types,
            )
            self.progress.emit("Density ready")
...
class _DensityTypeDialog(QDialog):
    """
    Small modal: pick one or more densities to load -- alpha, beta, total
    (alpha + beta), and/or spin (alpha - beta). Shared by the
    NBO/fchk/molden orbital-picker dialogs, same rationale as
    _LocalizeOptionsDialog.

    Always opens, even for closed-shell systems -- there alpha == beta,
    so total is the only non-degenerate density and it's offered as the
    sole (locked-on) checkbox rather than skipping the dialog outright.
    """
    ...
    _OPTIONS = (
        ...
        ("spin", "Spin density (alpha − beta)"),
    )

    def __init__(self, is_open_shell, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Choose Density")
        ...
        self._checkboxes = {}
        if is_open_shell:
            layout.addWidget(QLabel("Choose one or more densities to load:"))
            offered = self._OPTIONS
        else:
            layout.addWidget(QLabel(
                "Closed-shell system — alpha equals beta, so total "
                "density is the only one available."
            ))
            offered = (("total", "Total density"),)
        ...
        if is_open_shell:
            self._checkboxes["spin"].setChecked(True)
        else:
            self._checkboxes["total"].setChecked(True)
            self._checkboxes["total"].setEnabled(False)

        btn_row = QHBoxLayout()
        ok_btn = QPushButton("Compute")
        ...
        def _sync_ok_enabled():
            ok_btn.setEnabled(any(cb.isChecked() for cb in self._checkboxes.values()))

        for cb in self._checkboxes.values():
            cb.toggled.connect(_sync_ok_enabled)
        _sync_ok_enabled()
    ...
    def selection(self):
        """Return the list of selected density_type strings."""
        return [
            key for key, _ in self._OPTIONS
...
class _OrbitalPickerDialog(QDialog):
    ...
    def __init__(self, basis_path: str, key_path: str, parent=None):
        ...
        self.key_path = key_path
        self._thread = None
        self._compute_start_time = None  # Timer for cube generation
    ...
    def _build_ui(self):
        ...
        spin_density_btn = QPushButton("Density…")
    ...
    def _open_spin_density_dialog(self):
        dlg = _DensityTypeDialog(self.is_open_shell, parent=self)
        _DialogCenterFilter.center(dlg)
        if dlg.exec_() != QDialog.Accepted:
            return
        ...
        self.compute_btn.setEnabled(False)
        self.status_label.setText(f"Computing {', '.join(density_types)} density…")

        # Same .47/.40 requirement as localization -- see _open_localize_dialog.
        basis47 = os.path.splitext(self.basis_path)[0] + ".47"

        self._sd_thread = _SpinDensityComputeThread(
            basis47, grid_quality, ext_dist, density_types, parent=self)
    ...
    def _on_spin_density_done(self, result):
        ...
        cubes = result["cubes"]
        self.status_label.setText(f"✓ {', '.join(sorted(cubes))} density ready")
        self.cubes_ready.emit(list(cubes.values()))
        self.accept()
    ...
    def _start_compute(self):
        ...
        self._thread.finished.connect(self._on_compute_done)
        self._thread.error.connect(self._on_compute_error)
        self._compute_start_time = time.time()  # Start timer
        self._thread.start()
        
        # Close the parent key file picker dialog after computation starts
        parent = self.parent()
    ...
    def _on_compute_done(self, results):
        ...
        elapsed = time.time() - self._compute_start_time if self._compute_start_time else 0
        time_str = f"{elapsed:.1f}s" if elapsed < 60 else f"{elapsed/60:.1f}m"
        self.status_label.setText(f" {len(results)} orbital grid(s) ready  |  {time_str}")
...
class _FchkOrbitalPickerDialog(QDialog):
    ...
    def _open_spin_density_dialog(self):
        ...
        self._sd_thread = _SpinDensityComputeThread(
            self.fchk_path, grid_quality, ext_dist, density_types, parent=self)
        self._sd_thread.progress.connect(self.status_label.setText)
        self._sd_thread.finished.connect(self._on_spin_density_done)
        self._sd_thread.error.connect(self._on_compute_error)
        self._sd_thread.start()
...
class _MoldenOrbitalPickerDialog(QDialog):
    ...
    def _open_spin_density_dialog(self):
        ...
        self._sd_thread = _SpinDensityComputeThread(
            self.molden_path, grid_quality, ext_dist, density_types, parent=self)
        self._sd_thread.progress.connect(self.status_label.setText)
        self._sd_thread.finished.connect(self._on_spin_density_done)
        self._sd_thread.error.connect(self._on_compute_error)
        self._sd_thread.start()
        ...
        for bond in cube['bonds']:
            ...
            for start, end, col in segs:
                if np.linalg.norm(end - start) < 1e-6:
                    continue
                tube = pv.Line(start, end).tube(
                    radius=r, n_sides=BOND_TUBE_SIDES)
    ...
    def _update_selected_highlights(self):
        ...
        for i, idx in enumerate(self.selected_atoms):
            ...
            sph  = pv.Sphere(radius=bs_r * 1.45, center=pos,
                             theta_resolution=ATOM_SPHERE_RESOLUTION,
                             phi_resolution=ATOM_SPHERE_RESOLUTION)
...
"""
    ...
    def visualize(self):
        ...
        self._dialog_center_filter = _DialogCenterFilter(self.app)
        self.app.installEventFilter(self._dialog_center_filter)
